...
public final class BasicColourHistogram implements ColourHistogram {
    ...
    @Override
    public void fill(int[][] imagePixels) {
        final int w = imagePixels[0].length;
        for (int[] row : imagePixels) {
            for (int i=0; i<w; i++) {
                ColourHelper.getRgbValues(row[i], rgbValues);
    ...
    @Override
    public void findSimilarityCentre(int[][] segmentPixels, int[] centre) {
        ...
        for (int j=0 ; j<h ; j++) {
            for (int i=0 ; i<w ; i++) {
                ColourHelper.getRgbValues(segmentPixels[j][i], rgbValues);
                final int ri = rgbValues[RED_INDEX]/binWidth;
                final int gi = rgbValues[GREEN_INDEX]/binWidth;
                final int bi = rgbValues[BLUE_INDEX]/binWidth;
